...
def main() -> None:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Reorganizando imágenes"):
        ...
        # Buscar imagen en subcarpeta de continente (estructura: source_dir/continent/filename)
        src = (source_dir / continent / filename).resolve()
        
        # Si no se encuentra, intentar buscar directamente (fallback)
        if not src.exists():
            src = (source_dir / filename).resolve()
        
        if not src.exists():
            missing_files.append(f"{continent}/{filename}")
            continue
...
def _generate_splits(
    ...
    for scene_path in scene_dir.iterdir():
        ...
        for continent_path in scene_path.iterdir():
            ...
            # Verificar que no haya duplicados por nombre
            image_names = [img.name for img in images]
            if len(image_names) != len(set(image_names)):
                duplicates = [name for name in image_names if image_names.count(name) > 1]
                print(f"  ⚠️  Advertencia: {len(set(duplicates))} imágenes duplicadas por nombre en {continent_path.name}")
                ...
                print(f"  Imágenes únicas después de limpieza: {len(images)}")

            # Split estratificado (aunque solo hay una clase por carpeta, mantiene proporción)
            # Nota: train_test_split sin stratify está bien aquí porque cada carpeta es una clase
            train_imgs, test_imgs = train_test_split(
                images,
                test_size=test_size,
                random_state=seed,
                shuffle=True,  # Asegurar aleatoriedad
            )
            ...
            if overlap:
                raise ValueError(
                    f"ERROR CRÍTICO: {len(overlap)} imágenes aparecen en train Y test en {continent_path.name}:\n"
                    f"  Ejemplos: {list(overlap)[:5]}"
                )
        ...
        for c in os.listdir(train_dir):
            train_all_files.update(f.name for f in (train_dir / c).glob("*.*"))
        for c in os.listdir(test_dir):
            test_all_files.update(f.name for f in (test_dir / c).glob("*.*"))
        
        final_overlap = train_all_files & test_all_files
        if final_overlap:
            raise ValueError(
                f"ERROR CRÍTICO: {len(final_overlap)} archivos duplicados entre train y test en escena '{scene}':\n"
                f"  Ejemplos: {list(final_overlap)[:10]}"
            )
        
        print(
            f"Generados splits para escena '{scene}': {train_count} imágenes en train, {test_count} en test."
        )
        print(f"  ✓ Verificación: Sin solapamiento entre train y test")
